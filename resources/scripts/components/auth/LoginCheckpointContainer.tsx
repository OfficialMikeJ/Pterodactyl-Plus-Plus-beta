...
type Props = OwnProps & {
    ...
    onSuccess: (intended: string) => void;
};
...
const EnhancedForm = withFormik<Props, Values>({
    handleSubmit: ({ code, recoveryCode }, { setSubmitting, props: { clearAndAddHttpError, location, onSuccess } }) => {
        loginCheckpoint(location.state?.token || '', code, recoveryCode)
            .then((response) => {
                if (response.complete) {
                    // Hold on the pulsating brand splash before transitioning into the panel.
                    onSuccess(response.intended || '/');
                    return;
                }
